...
def train_lgd_model(df, sample_frac=0.15):
    m = df["TypeOfAccount"].eq("Loan") & df["LGD"].notna()
    d = df.loc[m].copy()
    ...
    drop_cols = [c for c in d.columns if c != "LGD" and any(p in c.upper() for p in ban)]
    d = d.drop(columns=drop_cols, errors="ignore")

    X, y, feats, pre = select_features(d, target="LGD")
    ...
    if "DateOfPortfolio" in d.columns and pd.api.types.is_datetime64_any_dtype(d["DateOfPortfolio"]):
        ...
        y_tr = d_sorted["LGD"].iloc[:cutoff].values
        X_va = d_sorted[feats].iloc[cutoff:]
        y_va = d_sorted["LGD"].iloc[cutoff:].values
        idx_va = X_va.index
