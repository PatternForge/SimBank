...
def train_pd_model(df, sample_frac=0.15):
    m = df["TypeOfAccount"].eq("Loan") & df["PD"].notna()
    d = df.loc[m].copy()
    ...
    drop_cols = [c for c in d.columns if c != "PD" and any(p in c.upper() for p in ban)]
    d = d.drop(columns=drop_cols, errors="ignore")

    X, y, feats, pre = select_features(d, target="PD")
    ...
    if "DateOfPortfolio" in d.columns and pd.api.types.is_datetime64_any_dtype(d["DateOfPortfolio"]):
        ...
        y_tr = d_sorted["PD"].iloc[:cutoff].values
        X_va = d_sorted[feats].iloc[cutoff:]
        y_va = d_sorted["PD"].iloc[cutoff:].values
        idx_va = X_va.index
